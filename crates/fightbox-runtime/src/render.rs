...
use fightbox_api::{
    CalibrationError, EngineConfig, ListenerState, SceneCalibration, SourceDrive, SourceError,
    SourceProfile,
};
    ...
    fn process_block(&mut self, block: ProcessBlock<'_>) -> Result<(), RenderError> {
        ...
        for input in block.sources {
            ...
            for frame in 0..block_size {
                ...
                source.calibrated[frame] = input.decoded_mono[frame] * drive.linear_gain();
...
mod tests {
    ...
    fn offline_driver_uses_calibrated_per_source_graph_and_stereo_bus() {
        ...
        for frame in 0..16 {
            assert!((left[frame] - drive.linear_gain()).abs() < 1.0e-6);
            assert!((right[frame] - drive.linear_gain() * 0.5).abs() < 1.0e-6);
        }
        assert_eq!(offline.processor().telemetry().timings.len(), 1);
    }
    ...
    fn backend_source_isolation_survives_muting_one_of_four_sources() {
        ...
        let inputs = [[0.125_f32; 16], [0.25; 16], [0.5; 16], [1.0; 16]];
