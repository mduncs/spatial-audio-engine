...
//!
//! Self-contained so the evidence layer never has to touch the workspace
//! lockfile. The algorithm is FIPS 180-4; the tests pin it to the standard
//! answer vectors, so a regression is caught locally.
...
pub fn sha256(data: &[u8]) -> [u8; 32] {
    ...
    // Pad to a multiple of 64 bytes, leaving 8 bytes for the big-endian bit length.
